    ...
    public boolean action(Request request, Response response) {
        if (request.getNettyRequest() instanceof FullHttpRequest) {
            ...
            if (inputAddGroupMember.isValide()) {
                RPCCenter.getInstance().sendRequest(inputAddGroupMember.getOperator(), null, "", IMTopic.SetGroupManagerTopic, inputAddGroupMember.toProtoGroupRequest().toByteArray(), inputAddGroupMember.getOperator(), TargetEntry.Type.TARGET_TYPE_USER, new RPCCenter.Callback() {
                    @Override
